...
public class CategoryManagerController {
    ...
    public String ListCategory(Model model,
        ...
        Page<Category> page = categoryRepository.search(searchKeyword, categoryType, pageable);

        // Get parent list (only same type AND root categories)
    ...
    public String add(Model model,
        ...
        if (errors.hasErrors()) {
            ...
            if (image != null && !image.isEmpty()) {
                try {
                    String filename = fileUploadUtils.saveImage(image, "categories");
    ...
    private void updateCategoryStatusRecursive(Category category, boolean status) {
        ...
        // Nếu là danh mục sản phẩm (type = true), ẩn luôn các sản phẩm thuộc danh mục
        // này
        if (Boolean.TRUE.equals(category.getType())) {
            productRepository.updateStatusByCategoryId(category.getId(), status);
        }
